...
public class OpenAiProvider : IContentGenerationProvider
{
    ...
    public async Task<ChatCompletionResult> CompleteAsync(ChatCompletionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var debugPayload = JsonSerializer.Serialize(new Dictionary<string, object?>
            {
                ["sessionId"] = "d9194e",
                ["hypothesisId"] = "A",
                ["location"] = "OpenAiProvider.CompleteAsync:result",
                ["message"] = "openai completion finish_reason",
