...
>
> - **No database, no EF Core, no repository pattern.** State lives in a plain in-process object store (`ProjectStore`/`ClientStore` in `ContentWriter.Infrastructure/InMemory/`), for the process lifetime only. This isn't a stopgap or a preference — two earlier database-backed designs for storing/serving content were tried and vetoed because they couldn't support the fine-grained per-element access this pipeline needs (e.g. `entry.sections[1].heading`). Relational/nested DB storage fought that access pattern; the structured `ContentDocument` tree (see below) doesn't.
> - **"Markdown export — retired" below is wrong — it's the opposite.** `.html` export (migrated from `.mdx` on 2026-07-21), committed directly to the geekatyourspot GitHub repo via `GeekatyourspotCommitService` (Git Data API), is now the *entire* persistence mechanism. It supersedes the GeekBackend-publish flow this doc describes throughout.
...
> - **Content pipeline rewritten 2026-07-22 to eliminate Markdown entirely, not just the file extension.** `.mdx` → `.html` (above) only fixed the artifact's name; the body was actually Markdown end-to-end until this rewrite (LLM prompted for GitHub-Flavored Markdown, Markdig parsed `##` headings for section trees, export ran `Markdown.ToHtml()` before wrapping in YAML frontmatter). Replaced with structured `Section`/`Paragraph`/`Run` records (`ContentWriter.Domain.Entities.ContentDocument`) — the LLM returns JSON, never markup or Markdown syntax; `entry.sections[1].heading`-style addressing is plain object indexing, no parsing. Export (`HtmlExportService`/`SectionHtmlRenderer`) builds a real DOM (HtmlAgilityPack) and serializes a standalone `<!doctype html>` document — no YAML frontmatter, no Keystatic contract, no Markdig. `MdxExportService`, `ArticleHtmlSectionExtractor`, `ToolsSectionHtmlParser`, `HtmlBodyNormalizer` are deleted; do not reintroduce Markdown/Markdig anywhere in this pipeline.
---
todos:
  - id: "v2-skeleton"
...
- **Word count:** undershoot triggers regeneration passes, capped at 3 attempts; overshoot is not treated as a failure. Applies wherever a floor exists (pillar, blog, tool pages). The ported v1 trim pass is dropped/relaxed accordingly. Attempt count is tracked per content row and exposed in the batch progress API (same pattern as `ReviewVerdict.AttemptCount`). Exceeding the cap fails the item into an error state — visible in the dashboard, never silently published under the floor.
...
- **Fully automated revise loop, no human gate:** reviewer direction feeds regeneration, then re-review, repeating until approved or the attempt cap is hit. No "held for manual review" state and no notification path.
- **Live revision counter:** the loop increments `ReviewVerdict.AttemptCount` on every pass and the batch progress API exposes it per content row, so the dashboard displays a running "revision N" counter while a row is in review.
- **Loop exit condition: capped at 3 attempts.** Exceeding the cap fails the item into an error state (visible in the dashboard via the revision counter) rather than spinning indefinitely on a pathological keyword. Phase 6 still captures convergence data (attempt counts, cost per keyword, any item that hits the cap) — that data now informs whether 3 is the right number long-term, not whether a cap is needed at all.
